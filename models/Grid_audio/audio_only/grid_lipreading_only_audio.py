...
import argparse
import os
from lipreading_utils import find_data_mean, find_data_std, HDF5AudioOnlyLipReadingIterator, ftensor5, get_train_and_val_fn, vocab_size, print_model
from sensors_ini.rnns.lasagne_utils import save_model, store_in_log, load_model, load_log, replace_updates_nans_with_zero
from lipreading_models import get_dan_audio_only
import h5py
...
if __name__ == '__main__':
    ...
    parser.add_argument('--h5file',       default='/home/dneil/datasets/grid/grid_40ms.hdf5', help='HDF5 File that has the data.')
    ...
    parser.add_argument('--filename',     default='lipreading_28.8.16', help='Filename to save model and log to.')
    ...
    parser.add_argument('--wait_period',  default=20, type=int, help='How long to wait before looking for early stopping.')
    ...
    d = HDF5AudioOnlyLipReadingIterator()
    ...
    network = get_dan_audio_only(aud_in)
    # Instantiate log
    ...
    for epoch in range(args.num_epochs):
        ...
        for data in d.flow(dataset, 'train', batch_size=args.batch_size, shuffle=True):
            ...
            aud_in, aud_mask, bY = data
            # Do a training batch
            try:
                calc_start_time = time.time()
                err, acc = train_fn(aud_in, bY)
