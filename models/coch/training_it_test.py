...
import argparse
import os
from lipreading_utils import pad_sequences,find_data_mean, find_data_std, HDF5LipReadingIterator_sep, HDF5LipReadingIterator_events,ftensor5, get_train_and_val_fn, vocab_size, print_model
from lasagne_utils import save_model, store_in_log, load_model, load_log, replace_updates_nans_with_zero
from lipreading_models import get_dan_original_rnn,get_network,get_network_12classes,get_dan_original_audio,get_network_coch
import h5py
...
#h5file = '/home/xiaoya/lipreading/data/lipread_data/grid_coch_recap/grid_coch_ctime_40_on.hdf5'
h5file_video = '/home/xiaoya/lipreading/data/grid/hdf5/grid_40ms.hdf5'
h5file_coch='/home/xiaoya/lipreading/data/grid_coch_recap/grid_coch_ctime_40_on.hdf5'
test_run_id='0'
# Get dataset
...
network = get_network_coch(aud_in)
...
train_fn, val_fn, out_fn = get_train_and_val_fn([aud_in], targets, network)
print('Compiled.')
...
filename = 'lipreading_grid_coch_ctime_40_on_it_test'
new_filename=filename+'_1'
old_filename='lipreading_recap_events_0_final'
blankout=0
aud_blankout=0
...
for epoch in range(num_epochs):
    ...
    for data in d.flow(dataset_video,dataset_coch, 'train', batch_size=batch_size, shuffle=True, blankout=blankout,
        ...
        try:
            ...
            err, acc,debug_var = train_fn(aud_in, bY)
    ...
    for data in d.flow(dataset_video,dataset_coch, 'test', batch_size=batch_size, shuffle=False, blankout=test_blankout):
        ...
        try:
            calc_start_time = time.time()
            err, acc,debug_var2 = val_fn(aud_in,  bY)
