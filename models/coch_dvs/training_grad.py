...
import argparse
import os
from lipreading_utils_grad import pad_sequences,find_data_mean, find_data_std, HDF5LipReadingIterator_shift, HDF5LipReadingIterator_events,HDF5LipReadingIterator_sep,ftensor5, get_train_and_val_fn, vocab_size, print_model
...
h5file_video = '/home/xiaoya/lipreading/data/lipread_data/current_used_datasets/DVS_corr_2.hdf5'
...
filename = 'lipreading_dvs_coch_shift_corr'
...
for epoch in range(num_epochs):
    ...
    data_start_time = time.time()
    for data in d.flow(dataset_video,dataset_coch, 'train', batch_size=batch_size, shifts_per_sample= shifts, shuffle=True, blankout=blankout,
        ...
        # Log and print
        log = store_in_log(log, {'b_train_err': err, 'b_train_acc' : acc, 'b_grads_normed' : grads_normed,'b_activations':actvivations})
    ...
    val_batches = 0
    data_start_time = time.time()
    for data in d.flow(dataset_video,dataset_coch, 'test', batch_size=batch_size, shifts_per_sample= shifts,shuffle=False, blankout=test_blankout):
        aud_in, aud_mask, vid_in, vid_mask, bY = data
        print(bY.shape)
        try:
            calc_start_time = time.time()
