...
#h5file_audio = '/home/xiaoya/lipreading/data/lipread_data/grid_40ms.hdf5'
h5file_audio='/home/xiaoya/lipreading/Grid_audio.hdf5'
h5file_video='/home/xiaoya/lipreading/Grid_video.hdf5'
test_run_id='0'
# Get dataset
...
filename = 'lipreading_Grid_datasetII'
new_filename=filename+'_leaky'
...
num_train_batches = int(np.ceil(float(len(dataset_video['train_labels']))/batch_size))
num_test_batches = int(np.ceil(float(len(dataset_video['test_labels']))/batch_size))
...
for epoch in range(num_epochs):
    ...
    data_start_time = time.time()
    for data in d.flow(dataset_video,dataset_audio, 'train', batch_size=batch_size, shuffle=True, blankout=blankout,
        ...
        try:
            ...
            err, acc,debug_var = train_fn(aud_in,vid_in, bY)
    ...
    val_batches = 0
    data_start_time = time.time()
    for data in d.flow(dataset_video,dataset_audio, 'test', batch_size=batch_size, shuffle=False, blankout=test_blankout):
        aud_in, aud_mask, vid_in, vid_mask, bY = data
        print(bY.shape)
        try:
            calc_start_time = time.time()
    ...
    log = store_in_log(log, {'val_err': val_err / val_batches,
                             ...
                             'val_no_vid_acc':val_no_vid_acc/val_batches*100.,
                             'val_no_aud_acc':val_no_aud_acc/val_batches*100.
                             } )
