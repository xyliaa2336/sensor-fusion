...
fopen(handles.u1);
handles.u2=udp('localhost',8999); % make a UDP interface to localhost on port 8997 (the default port for AEViewer RemoteControl interface)
fopen(handles.u2);
%coch ams1c
handles.u3=udp('localhost',8998); % make a UDP interface to localhost on port 8997 (the default port for AEViewer RemoteControl interface)
fopen(handles.u3);
...
delete(handles.u2);
fclose(handles.u3); % clean up the UDP connection
delete(handles.u3);
%fclose(handles.Beep);
...
%              filesep handles.tid filesep wlist{cnt} ret_suffix])
        ...
        commandJAER(handles.u2,'zerotimestamps')
        commandJAER(handles.u3,'zerotimestamps')
...
%          commandJAER(handles.u2,'stoplogging')
        commandJAER(handles.u1,'togglesynclogging')
        movefile([default_jpath 'CochleaLP*'], [filename]);
