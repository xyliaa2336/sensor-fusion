...
dest_dir='D:\lipread_data\hw_grid_DVS_part3\'; % directory to save sensor files
...
bbox = step(faceDetector, videoFrame);
            box_range=[bbox(2)-80,bbox(2)+bbox(4)+80,bbox(1)-100,bbox(1)+bbox(3)+100];
...
%             bbox            = step(faceDetector, videoFrame);
    ...
    %         commandJAER(handles.u1,'togglesynclogging')
            ...
            for i=1:numFrames
                ...
                imagesc(mov(i).cdata);
                %set(hf, 'position', [2250 -100 size(mov(1).cdata,1) size(mov(1).cdata,2)])
                pause(1/readerobj.FrameRate-toc);
            end
