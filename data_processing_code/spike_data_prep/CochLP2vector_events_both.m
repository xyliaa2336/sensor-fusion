...
rootpath='F:\hw_grid_mat\CochLP_events_both\';
...
for i=1:size(coch_list,1)
   ...
   time_steps=1:floor(max(length(tsleftoff),length(tslefton))/num_events)+1;
   if(mod(i,1000)==0)
		i
   end
...
%    end
   ...
   for j=1:length(time_steps)
       ...
       if(window_start>length(tsleftoff)) %only on events now
           ind=window_start:min(window_start+num_events-1,length(tslefton));
           for ind_cnt=1:length(ind)
                    coch_vect(addrlon(ind(ind_cnt))+1,time_steps(j))=coch_vect(addrlon(ind(ind_cnt))+1,time_steps(j))+1;
           end
       elseif(window_start>length(tslefton)) %only off events now
       ...
       elseif(window_start>length(tsleftoff) && window_start>length(tslefton)) %just a sanity check
           break
       else
           indon=window_start:min(window_start+num_events-1,length(tslefton));
           ...
           indoff=window_start:min(window_start+num_events-1,length(tsleftoff));
