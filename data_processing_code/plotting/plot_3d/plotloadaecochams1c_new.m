...
function [tsleft, laddress, tsright, raddress]=plotloadaecochams1c_new(allTs, allAddr)
...
%allTsd2=double(allTs-min(allTs))/1e6;
allTsd2=double(allTs)/1e6;

%find spikes, not ADC values
cochlea_id=double(bitshift((bitand(allAddrT,cochleaaddrmask)), -13)); %cochleaid=0
allAddr=allAddrT(cochlea_id==0);
allTsd=allTsd2(cochlea_id==0);
...
%allTsd=double(allTs-min(allTs))/1e6;
...
if ((length(tsleft)>0) | (length(tsright)>0))
    ...
    raddress=raddress+offset; raddressinteger=raddressinteger+offset;
    ...
    plot (tsright, raddress, '.', 'MarkerSize', 10, 'Color', [0 0.6 0])
    hold on
    plot (tsrightinteger,raddressinteger, '.', 'MarkerSize', 10, 'Color',[0 0.6 0])% [0.6 0.6 0.6])
    hold off
    xlabel('Time')
    %xlim ([0 Time]);
    %            ylim ([0 33]);
