% Read APS frames from DAVIS sensor outputs
% Inputs:
% file -> file name (string)
% [x0, y0, x1, y1] = region of interest (pixels) -> optional
% Bugfixes by Soumyajit Mandal, 07/14/15

function [frames, frameEndsTs] = getAPSframesDavisGS(file, x0, y0, x1, y1)  
...
frames(5, :, :, :) = readTsBuffer;
frames(6, :, :, :) = exposures;

frameEndsTs = framedataTs(frameEnds(2:end)); % last event time in each frame
