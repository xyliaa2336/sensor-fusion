%%% This function load cochlea + DAVIS + audio data
%%% recordings and extract the word segment
function get_word_from_recording_beep(basefil,word,des_dir)
    debug=0;
    % load data
...
%    in case time stamp is reset after recording started
    ...
    mic_threshold_coch=(max(aud_eng)-mean(aud_eng(1:5000)))/5+mean(aud_eng(1:5000))
    ...
    start_coch=allTsADC0(find(ind_coch,1));
